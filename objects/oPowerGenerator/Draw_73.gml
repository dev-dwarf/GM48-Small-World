	...
	if (target != noone and instance_exists(target)) {
		...
		if (target == oDoor) {
				...
				for (var i = 0; i < dist/(7*_scale); i++) {
					draw_sprite_ext(sLightning, other.lightning_index, x + vx * i, y -51 + vy * i, _scale, _scale * min(1.0, max(i/16, 0.25)), dir + irandom_range(-5, 5), make_color_rgb(0, 205, 249), 0.9);
